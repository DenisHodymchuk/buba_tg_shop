...
-- Update orders status check constraint to support the new 'shipped' and 'painting' statuses
ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_status_check;
ALTER TABLE orders ADD CONSTRAINT orders_status_check CHECK (status IN ('new', 'preparing', 'printing', 'painting', 'shipping', 'shipped', 'completed', 'cancelled'));
